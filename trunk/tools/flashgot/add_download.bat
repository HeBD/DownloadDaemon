...
rem If you call this from flashgot, use the following options: Program-path: cmd.exe
rem Parameters: /C <path to script> <path to ddconsole.exe> <host> [UFILE] <password>
rem password is optional. This only works on linux and with DownloadDaemon 0.9 and above
...
IF /I ["%PASSWORD%"]==[""] (%DDCONSOLE% --host "%HOST%" --command "pkg add" > c:\tmp_flashgot) ELSE (%DDCONSOLE% --host "%HOST%" --password "%PASSWORD%" --command "pkg add" > c:\tmp_flashgot)
