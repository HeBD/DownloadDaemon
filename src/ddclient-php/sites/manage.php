...
connect_to_daemon($socket, $_COOKIE['ddclient_host'], $_COOKIE['ddclient_port'], $_COOKIE['ddclient_passwd'], $_COOKIE['ddclient_enc'], 5);
	if(isset($_POST['activate'])) {
		...
		if(mb_substr($buf, 0, 3) != "100") {
			$dl_list .= "Could not activate download " . $_POST['id'];
			exit;
		}
	}
	...
	if(isset($_POST['deactivate'])) {
		...
		recv_all($socket, $buf);
		if(mb_substr($buf, 0, 3) != "100") {
			$dl_list .= "Could not deactivate download " . $_POST['id'];
			exit;
		}
	}
	...
	if(isset($_POST['delete'])) {
		...
		recv_all($socket, $buf);
		if(mb_substr($buf, 0, 3) != "100") {
			$dl_list .= "Could not delete download " . $_POST['id'];
			exit;
		}
	}
	...
	if(isset($_POST['delete_file'])) {
		...
		recv_all($socket, $buf);
		if(mb_substr($buf, 0, 3) != "100") {
			$dl_list .= "Could not delete File " . $_POST['id'];
			exit;
		}
	}
	...
	if(isset($_POST['up'])) {
		...
		recv_all($socket, $buf);
		if(mb_substr($buf, 0, 3) != "100") {
			$dl_list .= "Could not move download upwards";
			exit;
		}
	}
